...
/**
  * 不使用kafkaStream.transform的方法获取偏移量
  * 而是直接操作kafkaStream，遍历其中的kafkaRDD，强转成HasOffsetRanges获取偏移量
  */
object KafkaDirectWordCount5 {
  def main(args: Array[String]): Unit = {
    ...
    kafkaStream.foreachRDD {
      kafkaRDD => {
        ...
        val resulted: RDD[(String, Int)] = lines.flatMap(_.split(" ")).map((_, 1)).reduceByKey(_ + _)
        println(resulted.collect().toBuffer)
